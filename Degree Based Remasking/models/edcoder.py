...
class PreModel(nn.Module):
    def __init__(
            ...
            max_epoch: int = 1000, # Added max_epoch for curriculum learning safety
         ):
        ...
        self._max_epoch = max_epoch # Store max_epoch
    ...
    def mask_attr_prediction(self, g, x, targets, epoch, drop_g1=None, drop_g2=None):
        ...
        if self._remask_method == "random":
            for i in range(self._num_remasking):
                ...
                # ! MODIFICATION: Passed epoch to remask function
                rep, remask_nodes, rekeep_nodes = self.curriculum_remask(use_g, rep, self._remask_rate, epoch)
    ...
    def curriculum_remask(self, g, rep, remask_rate, epoch):
        """
        Implements Smart/Curriculum Masking based on degree centrality.
        Early epochs: Prefer masking low-degree (unimportant) nodes.
        Later epochs: Prefer masking high-degree (hub) nodes.
        """
        num_nodes = g.num_nodes()
        device = rep.device
        ...
        # 2. Calculate Curriculum Progress (alpha)
        # alpha ranges from -1 (at epoch 0) to +1 (at max_epoch)
        # alpha < 0: weights ~ 1/degree (mask low degree)
        # alpha > 0: weights ~ degree (mask high degree)
        # alpha = 0: weights ~ 1 (random)
        
        # Ensure we don't exceed max_epoch for the calculation
        cur_epoch = min(epoch, self._max_epoch)
        progress = cur_epoch / self._max_epoch
        alpha = 2.0 * progress - 1.0

        # 3. Calculate weights
        # We use the power of degrees as unnormalized probabilities
        weights = torch.pow(degrees, alpha)

        # 4. Sample nodes
        remask_nodes = torch.multinomial(weights, num_remask_nodes, replacement=False)

        # 5. Determine keep nodes
        # Create boolean mask for efficient complement finding
        mask = torch.zeros(num_nodes, dtype=torch.bool, device=device)
        mask[remask_nodes] = True
        rekeep_nodes = torch.where(~mask)[0]

        # 6. Apply Masking to Representations
        rep = rep.clone()
        rep[remask_nodes] = 0
