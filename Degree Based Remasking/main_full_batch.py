...
def pretrain(model, graph, feat, optimizer, max_epoch, device, scheduler, num_classes, lr_f, weight_decay_f, max_epoch_f, linear_prob, logger=None):
    ...
    for epoch in epoch_iter:
        ...
        # ! FIXED: Passing 'epoch' to the model for curriculum masking
        loss = model(graph, x, targets=target_nodes, epoch=epoch)
...
def main(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        # ! CHANGED: Unpacking 4 values now instead of 2
        final_acc, estp_acc, final_auc, estp_auc = linear_probing_full_batch(model, graph, x, num_classes, lr_f, weight_decay_f, max_epoch_f, device, linear_prob)
    ...
    # ! NEW: Calculate and print AUC stats
