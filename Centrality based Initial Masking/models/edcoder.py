...
class PreModel(nn.Module):
    ...
    def mask_attr_prediction(self, g, x, targets, epoch, drop_g1=None, drop_g2=None):
        # ! MODIFIED: Using Degree-Based Masking here
    ...
    def encoding_mask_noise(self, g, x, mask_rate=0.3):
        # ! NEW LOGIC: Degree-Based Masking for Input
        ...
        # 2. Calculate weights based on degree (Higher degree = Higher probability)
        weights = degrees

        # 3. Sample nodes using multinomial distribution based on weights
        # replacement=False ensures we don't pick the same node twice
        mask_nodes_idx = torch.multinomial(weights, num_mask_nodes, replacement=False)
        
        # 4. Determine keep nodes (complement of mask nodes)
        ...
        # Add mask token to masked nodes
