...
from sklearn.metrics import roc_auc_score # Added for AUC
...
def compute_auc(y_pred, y_true):
    ...
    Handles both binary and multi-class (One-vs-Rest) cases.
    """
    y_true = y_true.squeeze().long().detach().cpu().numpy()
    ...
    try:
        if num_classes == 2:
            # Binary case: expect probability of the positive class
        ...
        # Handle cases where a split might miss a class causing sklearn issues
