...
class PreModel(nn.Module):
    ...
    def mask_attr_prediction(self, g, x, targets, epoch, drop_g1=None, drop_g2=None):
        # ! MODIFIED: This now calls the PageRank-based masking logic
        ...
        loss_rec_all = 0
        # REVERTED Re-masking to original random logic as requested
    ...
    def encoding_mask_noise(self, g, x, mask_rate=0.3):
        # ! NEW LOGIC: PageRank-Based Initial Masking
        num_nodes = g.num_nodes()
        device = x.device
        num_mask_nodes = int(mask_rate * num_nodes)

        # 1. Compute PageRank (if not cached)
        if self._pagerank_scores is None:
            try:
                import networkx as nx
                # Need to move graph to CPU for NetworkX conversion
                nx_g = dgl.to_networkx(g.cpu())
                pr_scores = nx.pagerank(nx_g, alpha=0.85)
                
                # Convert dict to tensor on correct device
                scores_tensor = torch.zeros(num_nodes, device=device)
                for node_id, score in pr_scores.items():
                    scores_tensor[node_id] = score
                self._pagerank_scores = scores_tensor
            except Exception as e:
                print(f"PageRank calculation failed: {e}. Using random masking.")
                self._pagerank_scores = torch.ones(num_nodes, device=device)

        # 2. Use PageRank scores as weights
        weights = self._pagerank_scores

        # 3. Sample nodes (High PageRank = High probability of being masked)
        mask_nodes_idx = torch.multinomial(weights, num_mask_nodes, replacement=False)
        
        # 4. Determine keep nodes
        full_mask = torch.zeros(num_nodes, dtype=torch.bool, device=device)
        full_mask[mask_nodes_idx] = True
        keep_nodes_idx = torch.where(~full_mask)[0]

        out_x = x.clone()
        out_x[mask_nodes_idx] = 0.0
        out_x[mask_nodes_idx] += self.enc_mask_token
        
        use_g = g.clone()

        return use_g, out_x, (mask_nodes_idx, keep_nodes_idx)
