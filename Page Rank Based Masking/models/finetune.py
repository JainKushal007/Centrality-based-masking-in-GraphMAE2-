...
from utils import accuracy, compute_auc, set_random_seed, show_occupied_memory, get_current_lr # Added compute_auc
...
def linear_probing_full_batch(model, graph, x, num_classes, lr_f, weight_decay_f, max_epoch_f, device, linear_prob=True, mute=False):
    ...
    final_acc, estp_acc, final_auc, estp_auc = _linear_probing_full_batch(encoder, graph, x, optimizer_f, max_epoch_f, device, mute)
    return final_acc, estp_acc, final_auc, estp_auc
...
def _linear_probing_full_batch(model, graph, feat, optimizer, max_epoch, device, mute=False):
    ...
    with torch.no_grad():
        ...
        test_auc = compute_auc(pred[test_mask], labels[test_mask])
        estp_test_auc = compute_auc(pred[test_mask], labels[test_mask]) 
        
    # Calculate for last epoch model 
    model.eval() 
    with torch.no_grad():
        pred_last = model(graph, x)
        final_test_auc = compute_auc(pred_last[test_mask], labels[test_mask])
    
    if mute:
        print(f"# IGNORE: --- TestAcc: {test_acc:.4f}, Best ValAcc: {best_val_acc:.4f} --- ")
    else:
        print(f"--- TestAcc: {test_acc:.4f}, TestAUC: {final_test_auc:.4f}, Best ValAcc: {best_val_acc:.4f}, Best Val AUC: {estp_test_auc:.4f} --- ")

    return test_acc, estp_test_acc, final_test_auc, estp_test_auc